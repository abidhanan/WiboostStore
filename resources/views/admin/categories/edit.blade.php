...
<div class="wiboost-font pb-12 max-w-4xl mx-auto">
    ...
    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-[2rem] shadow-lg shadow-[#bde0fe]/20 border-4 border-white p-8">
        ...
        <div class="mb-6">
            <label class="block text-sm font-black text-[#8faaf3] mb-3 ml-2">Deskripsi Layanan</label>
            <textarea name="description" rows="3"
                      class="w-full bg-[#f4f9ff] border-2 border-transparent focus:border-[#5a76c8] rounded-[1.5rem] px-6 py-4 text-[#2b3a67] font-black outline-none transition">{{ old('description', $category->description) }}</textarea>
        </div>

        <div class="mb-6">
            <label class="block text-sm font-black text-[#8faaf3] mb-3 ml-2">Slug (URL Kategori)</label>
            <input type="text" name="slug" required value="{{ old('slug', $category->slug) }}"
                   class="w-full bg-[#f4f9ff] border-2 border-transparent focus:border-[#5a76c8] rounded-[1.5rem] px-6 py-4 text-[#2b3a67] font-black outline-none transition">
        </div>

        <div class="mb-8">
            <label class="block text-sm font-black text-[#8faaf3] mb-3 ml-2">Ganti Ikon / Logo Kategori</label>
            
            @if($category->image)
                <div class="mb-4 ml-2">
                    <p class="text-[10px] font-black text-[#8faaf3] mb-2 uppercase tracking-widest">Logo Saat Ini</p>
                    <div class="w-24 h-24 bg-[#f0f5ff] rounded-2xl border-2 border-white shadow-inner flex items-center justify-center p-2 overflow-hidden">
