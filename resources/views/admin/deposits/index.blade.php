...
<div class="wiboost-font pb-12">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4 pl-2">
        <div>
            ...
            <p class="text-sm text-[#8faaf3] font-bold mt-1">Pantau dan setujui pengisian dompet digital pelanggan.</p>
        </div>
            ...
            <table class="w-full text-left whitespace-nowrap">
                <thead>
                    <tr>
                        ...
                        <th class="px-6 py-4 text-[10px] font-black text-[#8faaf3] uppercase tracking-widest">Nominal Top Up</th>
                        <th class="px-6 py-4 text-[10px] font-black text-[#8faaf3] uppercase tracking-widest text-center">Status</th>
                        <th class="px-6 py-4 text-[10px] font-black text-[#8faaf3] uppercase tracking-widest text-center">Aksi (Manual)</th>
                    </tr>
                </thead>
                <tbody class="divide-y-2 divide-dashed divide-[#f0f5ff]">
                    ...
                    <tr class="hover:bg-[#f4f9ff] transition-colors rounded-xl group">
                        ...
                        <td class="px-6 py-4">
                            <p class="font-black text-[#5a76c8] text-lg">+ Rp {{ number_format($deposit->amount, 0, ',', '.') }}</p>
                            <p class="text-[10px] font-black text-[#8faaf3] mt-0.5 bg-[#f0f5ff] inline-block px-2 py-1 rounded-md border border-white">{{ strtoupper($deposit->payment_method ?? 'Menunggu') }}</p>
                        </td>
                        <td class="px-6 py-4 text-center">
                            @if($deposit->payment_status == 'paid')
                                <span class="bg-[#e6fff7] text-emerald-500 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border border-white shadow-sm">Sukses</span>
                        ...
                        <td class="px-6 py-4 text-center">
                            @if($deposit->payment_status == 'unpaid')
                                <form action="{{ route('admin.deposits.update', $deposit->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menyetujui Top Up ini? Saldo user akan otomatis bertambah.')">
                                    ...
                                    <button type="submit" class="bg-[#5a76c8] text-white hover:bg-[#4760a9] px-5 py-2.5 rounded-full text-xs font-black transition-transform active:scale-95 shadow-md shadow-[#5a76c8]/30 border-2 border-white">
                                        Approve Manual
                                    </button>
                                </form>
                            @else
                                <span class="text-[#8faaf3] text-[10px] font-black uppercase bg-[#f0f5ff] px-3 py-1 rounded-full border border-white">Selesai</span>
                            @endif
                        </td>
                    </tr>
                    @empty
