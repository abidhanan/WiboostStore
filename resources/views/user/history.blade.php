...
<div class="wiboost-font pb-12 max-w-4xl mx-auto mt-4">
    ...
    @if(session('success'))
        <div class="bg-[#e6fff7] border-4 border-white text-emerald-500 px-6 py-4 rounded-[2rem] mb-8 font-black flex items-center gap-3 shadow-sm">
            <span class="text-2xl">OK</span> {{ session('success') }}
        </div>
    @endif
            ...
            <div onclick="openModal('modal-{{ $trx->id }}')" class="bg-white rounded-[2.5rem] p-6 shadow-lg shadow-[#bde0fe]/20 border-4 border-white flex flex-col md:flex-row justify-between items-start md:items-center hover:border-[#bde0fe] hover:-translate-y-1 transition-all group cursor-pointer">
                <div class="mb-4 md:mb-0 w-full md:w-2/3 pr-0 md:pr-4">
                    ...
                    @if($trx->has_order_input && filled($trx->order_input_summary))
                        <p class="text-sm text-[#8faaf3] font-bold truncate mt-1">Input Buyer: {{ $trx->order_input_summary }}</p>
                    @endif
                </div>
            ...
            <div id="modal-{{ $trx->id }}" class="fixed inset-0 flex items-center justify-center bg-[#2b3a67]/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 p-4" style="z-index: 99999;">
                ...
                <div class="relative bg-white w-full max-w-md rounded-[2.5rem] p-6 md:p-8 shadow-2xl transform scale-95 transition-transform duration-300 flex flex-col overflow-hidden max-h-[90vh]">
                    <div class="flex justify-between items-center mb-6 pb-4 border-b-2 border-dashed border-[#f0f5ff]">
                        ...
                        <button onclick="closeModal('modal-{{ $trx->id }}')" class="w-10 h-10 bg-[#ffe5e5] text-[#ff6b6b] hover:bg-[#ff6b6b] hover:text-white rounded-xl flex items-center justify-center transition-colors font-black border-2 border-white">X</button>
                    </div>
                    ...
                    <div class="flex-1 overflow-y-auto hide-scroll space-y-5 pr-1">
                        ...
                        @if($credentials)
                            <div class="bg-[#fffcf0] p-5 rounded-[1.5rem] border-2 border-amber-200 shadow-sm relative overflow-hidden">
                                <div class="relative z-10">
                                    <p class="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-3">Data Pesanan Kamu</p>
                                        ...
                                        <div class="mt-5 space-y-3">
                                            @if(!empty($credentials['tutorial_link']))
                                                <a href="{{ $credentials['tutorial_link'] }}" target="_blank" class="w-full bg-white border-2 border-[#bde0fe] hover:border-[#5a76c8] text-[#5a76c8] font-black text-xs py-3 rounded-xl flex items-center justify-center gap-2 transition-colors shadow-sm">
                                                    <span>Panduan</span> Cara Menggunakan Akun/Nomor Ini
                                                </a>
                                            @endif
                                            ...
                                            @if(isset($credentials['needs_otp']) && $credentials['needs_otp'] == true)
                                                <div class="bg-white p-4 rounded-xl border-2 border-amber-100 flex flex-col gap-2 shadow-sm">
                                                    <p class="text-[10px] font-black text-amber-600 leading-tight">Pesanan ini membutuhkan bantuan login tambahan. Silakan cek tutorial yang tersedia atau tunggu admin memproses aksesnya.</p>
                                                </div>
                                            @endif
                                        </div>
                                    @endif
                        ...
                        @if($trx->has_order_input)
                            <div>
                                <p class="text-[10px] font-black text-[#8faaf3] uppercase tracking-widest mb-2">Input Buyer</p>
                                <div class="space-y-3">
                    ...
                    <div class="mt-6 pt-4 border-t-2 border-dashed border-[#f0f5ff] shrink-0">
                        @if($isUnpaid)
                            ...
                            @else
                                <div class="mb-3 rounded-[1.5rem] border-2 border-amber-100 bg-[#fff5eb] px-5 py-4 text-center text-sm font-black text-amber-600 shadow-sm">
                                    Tagihan ini belum memiliki link pembayaran otomatis. Tunggu admin memproses atau gunakan metode pembayaran yang tersedia.
                                </div>
                            @endif
